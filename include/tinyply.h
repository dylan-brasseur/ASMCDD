...
namespace tinyply
{
    ...
    public:
        Buffer() = default;;
        explicit Buffer(const size_t size) : data(new uint8_t[size], delete_array()), size(size) { alias = data.get(); } // allocating
        explicit Buffer(uint8_t * ptr) { alias = ptr; } // non-allocating, todo: set size?
        uint8_t * get() { return alias; }
        [[nodiscard]] size_t size_bytes() const { return size; }
    };
    ...
    struct PlyElement
    {
        ...
        PlyElement(std::string  _name, size_t count) : name(std::move(_name)), size(count) {}
        std::string name;
        size_t size{};
    ...
    struct PlyFile
    {
         ...
         */
        [[nodiscard]] std::vector<PlyElement> get_elements() const;
        ...
        std::shared_ptr<PlyData> request_properties_from_element(const std::string & elementKey, 
            std::initializer_list<std::string> propertyKeys, uint32_t list_size_hint = 0);

        void add_properties_to_element(const std::string & elementKey, 
            std::initializer_list<std::string> propertyKeys,
            Type type,
            size_t count,
            uint8_t * data, 
            Type listType,
...
struct PlyFile::PlyFileImpl
{
    ...
    static size_t read_property_binary(const Type & t, const size_t & stride, void * dest, size_t & destOffset, std::istream & is);
    static size_t read_property_ascii(const Type & t, const size_t & stride, void * dest, size_t & destOffset, std::istream & is);
    ...
    static void read_header_text(std::string line, std::istream & is, std::vector<std::string> & place, int erase = 0);

    void write_header(std::ostream & os);
    void write_ascii_internal(std::ostream & os);
    void write_binary_internal(std::ostream & os);
    static void write_property_ascii(Type t, std::ostream & os, uint8_t * src, size_t & srcOffset);
    static void write_property_binary(Type t, std::ostream & os, uint8_t * src, size_t & srcOffset, const size_t & stride);
};
...
bool PlyFile::PlyFileImpl::parse_header(std::istream & is)
{
    ...
    while (std::getline(is, line))
    {
        ...
        std::string token;
        ls >> token;
        if (token == "ply" || token == "PLY" || token.empty()) continue;
...
{
    if (elements.empty()) throw std::runtime_error("no elements defined; file is malformed");
    ...
    const Type type, const size_t count, uint8_t * data, const Type listType, const size_t listCount)
{
    ...
    {
        PlyElement newElement = PlyElement(elementKey, count);
...
PlyFile::PlyFile() { impl = std::make_unique<PlyFileImpl>(); };
PlyFile::~PlyFile() = default;;
